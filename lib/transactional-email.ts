...
export async function sendPurchaseDownloadEmail(input: PurchaseEmail) {
  ...
  return sendEmail({
    ...
    text: `Your Aureon music is ready.\n\nOrder reference: ${input.orderNumber}\n\nEach purchased song can be downloaded once only. Your secure download link is valid for 72 hours from issue and expires after that time. Please download and save your music before the link expires.\n\n${textItems}\n\nIf a technical problem prevents your download, contact Aureon support and quote your order reference.`,
    html: `<div style="background:#050505;padding:32px;font-family:Arial,sans-serif;color:#f5f1e8;"><div style="max-width:640px;margin:0 auto;"><p style="letter-spacing:3px;color:#d8b85f;text-transform:uppercase;">Aureon Music Group</p><h1 style="font-size:32px;margin:10px 0 18px;">Your music is ready.</h1><p>Hello ${escapeHtml(input.customerName || 'music lover')},</p><p>Thank you for your purchase. Your order reference is <strong>${escapeHtml(input.orderNumber)}</strong>.</p><p><strong>Each purchased song can be downloaded once only.</strong> <strong>Your secure download link is valid for 72 hours from issue and expires after that time.</strong> Please download and save your music before the link expires. Opening this email does not use your download.</p><div style="margin-top:28px;">${itemHtml}</div><p style="margin-top:26px;color:#bcbcbc;">If a genuine technical problem prevents your download, contact Aureon support and quote your order reference.</p></div></div>`,
  });
}
