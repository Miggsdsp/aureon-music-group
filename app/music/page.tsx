...
type AlbumRecord = PublicRecord & { title:string; slug:string; artist?:string; artistName?:string; genre?:string; year?:string|number; releaseDate?:string; coverUrl?:string; coverImageUrl?:string; songCount?:number; songs?:unknown[] };
  ...
  return <PageShell title="Music" kicker="Catalogue">
    ...
    {loading ? <div className="store-empty"><h3>Loading music…</h3></div> : <>
      {albums.length > 0 && <section className="album-grid">{albums.map((album) => {
        const cover = album.coverImageUrl || album.coverUrl || '/images/branding/Aureon_Header_Logo.png';
