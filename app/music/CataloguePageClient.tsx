...
export default function MusicPage() {
  const { items: albums, loading: albumsLoading } = usePublishedCollection<AlbumRecord>('albums', [], true);
  const { items: songs, loading: songsLoading } = usePublishedCollection<SongRecord>('songs', [], true);
  ...
  return <PageShell title="Music" kicker="Catalogue">
    ...
    <p><Link href="/artists">Explore the artists</Link> · <Link href="/discover">Discover songs and playlists</Link></p>
    {loading ? <div className="store-empty"><h3>Loading music…</h3></div> : <>
      ...
      {songs.length > 0 && <section className="album-track-section"><h2>Discover songs from the catalogue</h2><div className="track-list">{songs.slice(0,6).map(song => <article className="track-row" key={song.id}><div><h3><Link href={`/songs/${song.slug || song.id}`}>{song.title}</Link></h3><p>{song.artistName || song.artist}</p></div></article>)}</div></section>}
