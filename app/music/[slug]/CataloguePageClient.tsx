...
export default function AlbumPage(){
 ...
 const {items:allSongs}=usePublishedCollection<SongRecord>('songs',[],true);
 const {items:allAlbums}=usePublishedCollection<AlbumRecord>('albums',[],true);
 ...
 const songs=allSongs.filter(song=>matchesAlbum(song,album)).sort((a,b)=>Number(a.trackNumber??999)-Number(b.trackNumber??999));
 return <main className="page-shell album-detail-page"><Header/>
  <section className="album-hero-detail"><div className="album-detail-cover"><ArtworkImage src={cover} alt={`${album.title} album artwork`} width={1000} height={1000}/></div><div className="album-detail-copy"><Link href="/music" className="back-link"><ArrowLeft size={16}/> Back to music</Link><p className="eyebrow">{album.genre||''} · {album.releaseDate||album.year||''}</p><h1>{album.title}</h1><h2>{artistSlug ? <Link href={`/artists/${artistSlug}`}>{artistName}</Link> : artistName}</h2><p>{album.description||''}</p><div className={conversionStyles.actions}><Link className="ghost-button" href="#album-tracks">Listen to the album →</Link>{artistSlug?<Link className="primary-button" href={`/artists/${artistSlug}`}>Discover {artistName} →</Link>:null}<Link className="primary-button" href="/account?mode=signup">Create free account →</Link></div><p className={conversionStyles.note}>Start with a public preview, then create a free account to save favourites and playlists.</p></div></section>
  <section className={styles.trackSection} id="album-tracks"><div className={styles.trackHeading}><ListMusic/><div><p className="eyebrow">Track List</p><h2>Songs in this album</h2></div></div>
