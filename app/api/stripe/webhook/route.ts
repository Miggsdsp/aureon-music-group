...
async function fulfilPaidCheckout(session:Stripe.Checkout.Session){
 ...
 const orderRef=adminFirestore.collection('orders').doc(session.id);const pendingSnapshot=await orderRef.get();const pending=pendingSnapshot.data()||{};
 if(pending.status==='paid'){await sendPaidOrderEmails(session.id);const context=analyticsContextFromStripe(session.metadata);if(context.analyticsConsent)await recordTrustedAnalyticsEvent({...context,eventType:'purchase_complete',entityType:String(pending.purchaseType||'order'),entityId:String((pending.songIds||pending.productIds||[])[0]||'order'),title:String(pending.songs?.[0]?.title||pending.products?.[0]?.name||'Aureon order'),revenueCents:session.amount_total||Number(pending.amountTotal||0),currency:session.currency||String(pending.currency||'eur'),memberId:String(pending.memberUid||'')},session.id);return;}
 const pendingItems=(Array.isArray(pending.items)?pending.items:[]) as PendingItem[];
 ...
 const customerEmail=session.customer_details?.email||session.customer_email||String(pending.customerEmail||'');const customerName=session.customer_details?.name||String(pending.customerName||`${session.metadata?.firstName||''} ${session.metadata?.surname||''}`.trim());const billingAddress=session.customer_details?.address;const deliveryAddress=pending.deliveryAddress||null;const country=deliveryAddress?.country||billingAddress?.country||'Not captured';const city=deliveryAddress?.city||billingAddress?.city||'';const postalCode=deliveryAddress?.postalCode||billingAddress?.postal_code||'';const customerPhone=session.customer_details?.phone||pending.customerPhone||session.metadata?.phone||'';const deviceType=session.metadata?.deviceType||'Not captured';const trafficSource=session.metadata?.trafficSource||session.metadata?.utmSource||'Direct';const paymentIntentId=typeof session.payment_intent==='string'?session.payment_intent:session.payment_intent?.id||'';const orderNumber=`AUR-${session.created}-${session.id.slice(-6).toUpperCase()}`;const stripeCustomerId=typeof session.customer==='string'?session.customer:session.customer?.id||'';const memberUid=String(pending.memberUid||session.metadata?.memberUid||'');
 ...
 await adminFirestore.runTransaction(async transaction=>{
  ...
  transaction.set(orderRef,{orderNumber,purchaseType,orderType:pending.orderType||session.metadata?.orderType||purchaseType,stripeCheckoutSessionId:session.id,stripeCustomerId,stripePaymentIntentId:paymentIntentId,memberUid,inventoryReservationId:reservationId,inventoryReservationStatus:reservationId?(inventoryIssue?'manual_review':'confirmed'):'legacy_no_reservation',customerEmail,customerName,customerPhone,customerCountry:country,country,customerCity:city,customerPostalCode:postalCode,deliveryAddress:deliveryAddress||null,deviceType,trafficSource,utmSource:session.metadata?.utmSource||'',utmMedium:session.metadata?.utmMedium||'',utmCampaign:session.metadata?.utmCampaign||'',landingPath:session.metadata?.initialLandingPage||session.metadata?.landingPath||'',currency:session.currency||'eur',amountTotal:session.amount_total||0,songIds:songs.map(song=>song.id),productIds:merchItems.map(item=>item.id),songs:songs.map(song=>({id:song.id,title:song.title,artist:song.artist,privateFilePath:song.privateFilePath,unitAmount:song.unitAmount,quantity:1})),products:merchItems.map(item=>({id:item.id,name:item.name,unitAmount:item.priceCents,quantity:item.quantity,size:item.size||'',colour:item.colour||''})),status:'paid',paymentStatus:session.payment_status,fulfilmentStatus:merchItems.length?(inventoryIssue?'manual_review':'awaiting_fulfilment'):'digital_complete',inventoryIssue:inventoryIssue||'',downloadStatus:songs.length?'available':'not_applicable',downloadPolicy:songs.length?'single-use':'not_applicable',emailStatus:'pending',receiptEmailStatus:'pending',downloadEmailStatus:songs.length?'pending':'not_applicable',purchaseEmailsPending:true,purchaseEmailRetryAt:Date.now(),paidAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()},{merge:true});
 ...
 // Both the winning transaction and duplicate deliveries resume persisted emails.
 await sendPaidOrderEmails(session.id);
 const analytics=analyticsContextFromStripe(session.metadata);
 if(analytics.analyticsConsent)await recordTrustedAnalyticsEvent({...analytics,eventType:'purchase_complete',entityType:purchaseType,entityId:songs[0]?.id||merchItems[0]?.id||'order',title:songs[0]?.title||merchItems[0]?.name||'Aureon order',revenueCents:session.amount_total||0,currency:session.currency||'eur',memberId:memberUid},session.id);
}
