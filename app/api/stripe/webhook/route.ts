...
async function fulfilPaidCheckout(session:Stripe.Checkout.Session){
 ...
 const customerEmail=session.customer_details?.email||session.customer_email||String(pending.customerEmail||'');const customerName=session.customer_details?.name||String(pending.customerName||`${session.metadata?.firstName||''} ${session.metadata?.surname||''}`.trim());const billingAddress=session.customer_details?.address;const deliveryAddress=pending.deliveryAddress||null;const country=deliveryAddress?.country||billingAddress?.country||'Not captured';const city=deliveryAddress?.city||billingAddress?.city||'';const postalCode=deliveryAddress?.postalCode||billingAddress?.postal_code||'';const customerPhone=session.customer_details?.phone||pending.customerPhone||session.metadata?.phone||'';const deviceType=session.metadata?.deviceType||'Not captured';const trafficSource=session.metadata?.trafficSource||session.metadata?.utmSource||'Direct';const paymentIntentId=typeof session.payment_intent==='string'?session.payment_intent:session.payment_intent?.id||'';const orderNumber=`AUR-${session.created}-${session.id.slice(-6).toUpperCase()}`;const siteUrl=(process.env.NEXT_PUBLIC_SITE_URL||'https://www.aureonmusicgroup.com').replace(/\/$/,'');
 ...
 const created=await adminFirestore.runTransaction(async transaction=>{
  ...
  transaction.set(adminFirestore.collection('customers').doc(customerEmail||session.id),{email:customerEmail,name:customerName,phone:customerPhone,country,city,postalCode,deliveryAddress:deliveryAddress||null,stripeCustomerId:typeof session.customer==='string'?session.customer:session.customer?.id||'',totalOrders:FieldValue.increment(1),lifetimeSpend:FieldValue.increment(session.amount_total||0),lastOrderAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp(),createdAt:FieldValue.serverTimestamp()},{merge:true});
  transaction.set(orderRef,{orderNumber,stripeCheckoutSessionId:session.id,stripePaymentIntentId:paymentIntentId,inventoryReservationId:reservationId,inventoryReservationStatus:reservationId?(inventoryIssue?'manual_review':'confirmed'):'legacy_no_reservation',customerEmail,customerName,customerPhone,customerCountry:country,country,customerCity:city,customerPostalCode:postalCode,deliveryAddress:deliveryAddress||null,deviceType,trafficSource,utmSource:session.metadata?.utmSource||'',utmMedium:session.metadata?.utmMedium||'',utmCampaign:session.metadata?.utmCampaign||'',landingPath:session.metadata?.landingPath||'',currency:session.currency||'eur',amountTotal:session.amount_total||0,songIds:songs.map(song=>song.id),productIds:merchItems.map(item=>item.id),songs:songs.map(song=>({id:song.id,title:song.title,artist:song.artist,privateFilePath:song.privateFilePath,unitAmount:song.unitAmount,quantity:1})),products:merchItems.map(item=>({id:item.id,name:item.name,unitAmount:item.priceCents,quantity:item.quantity,size:item.size||'',colour:item.colour||''})),status:'paid',paymentStatus:session.payment_status,fulfilmentStatus:merchItems.length?(inventoryIssue?'manual_review':'awaiting_fulfilment'):'digital_complete',inventoryIssue:inventoryIssue||'',downloadStatus:songs.length?'available':'not_applicable',downloadPolicy:songs.length?'single-use':'not_applicable',emailStatus:'pending',paidAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()},{merge:true});
