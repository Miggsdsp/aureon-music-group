...
export default function SongPage() {
  ...
  const { items: songs } = usePublishedCollection<SongRecord>('songs', [], true);
  ...
  const description = song.description || details.description || song.story || details.story || '';
    ...
    <main className="page-shell song-detail-page">
      ...
      <section className="album-hero-detail">
        ...
        <div className="album-detail-copy">
          ...
          <h1>{title}</h1>
          <h2>{artistSlug ? <Link href={`/artists/${artistSlug}`}>{artist}</Link> : artist}</h2>
          {description && <p>{description}</p>}
          <div className="song-detail-meta" aria-label="Song information">
            {song.releaseDate && <span>Released {String(song.releaseDate).slice(0,10)}</span>}
