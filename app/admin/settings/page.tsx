...
export default function SettingsPage() {
  ...
  async function uploadBrandAsset(file: File, key: 'headerLogoUrl' | 'footerLogoUrl' | 'faviconUrl') {
    const path = `public/branding/${Date.now()}-${safeName(file.name)}`;
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage('Website, branding, homepage and legal settings saved. Public listeners update automatically.');
    } catch (error) {
  ...
  const select = (label: string, key: keyof Settings, items: Row[], empty: string) => (
    <label>{label}<select value={String(settings[key] || '')} onChange={event => setSettings({ ...settings, [key]: event.target.value })}>
  ...
  const upload = (label: string, key: 'headerLogoUrl' | 'footerLogoUrl' | 'faviconUrl', accept = 'image/*') => (
    <label>{label}<input type="file" accept={accept} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadBrandAsset(file, key); }} />
  ...
  return (
    <AdminShell>
      <div className="admin-page-heading">
        ...
        <p>Operate public website features, homepage selections, brand assets and legal-page relationships without editing code.</p>
      </div>
      ...
      <form className="admin-cms-form" style={{ maxWidth: 1080 }} onSubmit={save}>
        <fieldset>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 12 }}><input style={{ width: 18, height: 18 }} type="checkbox" checked={settings.merchandiseEnabled} onChange={event => setSettings({ ...settings, merchandiseEnabled: event.target.checked })} /><span>Merchandise enabled</span></label>
        </fieldset>

        <fieldset>
          <legend>Logos and branding assets</legend>
          <div className="checkout-fields two-columns">
            {upload('Header logo', 'headerLogoUrl')}
            {upload('Footer logo', 'footerLogoUrl')}
          </div>
