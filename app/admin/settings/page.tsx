...
type Settings = {
  merchandiseEnabled: boolean; siteName: string; supportEmail: string; announcement: string;
...
const defaults: Settings = {
  merchandiseEnabled: false, siteName: 'Aureon Music Group', supportEmail: '', announcement: '',
...
export default function SettingsPage() {
  ...
  async function uploadAsset(file: File, key: AssetKey) {
    if (key === 'heroVideoUrl' && file.type !== 'video/mp4') {
      setMessage('Please upload an MP4 video.'); return;
    }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage('Settings saved and published. The homepage will now use the selected video and overlays.');
    } catch (error) {
  ...
  return <AdminShell><div className="admin-page-heading"><p className="admin-kicker">Aureon Control Center</p><h1>Site Settings & Branding</h1><p>Operate public website features, homepage media, brand assets and legal-page relationships without editing code.</p></div>{message && <div className="admin-cms-message" role="status">{message}</div>}<form className="admin-cms-form" style={{ maxWidth: 1080 }} onSubmit={save}>
    <fieldset><legend>Platform</legend><div className="checkout-fields two-columns"><label>Public site name<input required value={settings.siteName} onChange={event => setSettings({ ...settings, siteName: event.target.value })} /></label><label>Support email<input type="email" value={settings.supportEmail} onChange={event => setSettings({ ...settings, supportEmail: event.target.value })} /></label></div><label>Announcement banner<textarea value={settings.announcement} onChange={event => setSettings({ ...settings, announcement: event.target.value })} placeholder="Leave blank to hide the announcement." /></label><div className="admin-feature-toggle"><div><strong>Merchandise page</strong><span>{settings.merchandiseEnabled ? 'Visible in the public navigation and available to customers.' : 'Hidden from the public navigation and unavailable to customers.'}</span></div><label className="admin-switch"><input type="checkbox" checked={settings.merchandiseEnabled} onChange={event => setSettings({ ...settings, merchandiseEnabled: event.target.checked })} /><span className="admin-switch-track"><span className="admin-switch-thumb" /></span><b>{settings.merchandiseEnabled ? 'ON' : 'OFF'}</b></label></div></fieldset>

    <fieldset><legend>Homepage Hero Manager</legend><p>Change the background media while keeping the Aureon logo, slogan, buttons and navigation as separate overlays.</p><div className="checkout-fields two-columns">{upload('Background video', 'heroVideoUrl', 'video/mp4', 'MP4 only. Upload first, then save all settings.')}{upload('Poster / fallback image', 'heroPosterUrl', 'image/jpeg,image/png,image/webp', 'Shown while the video loads or when autoplay is unavailable.')}</div><div className="checkout-fields two-columns"><label>Overlay darkness: {settings.heroOverlayOpacity}%<input type="range" min="0" max="100" value={settings.heroOverlayOpacity} onChange={event => setSettings({ ...settings, heroOverlayOpacity: Number(event.target.value) })} /></label><label>Homepage logo size: {settings.heroLogoScale}%<input type="range" min="60" max="150" value={settings.heroLogoScale} onChange={event => setSettings({ ...settings, heroLogoScale: Number(event.target.value) })} /></label></div>{effectToggle('Animated light effects', 'heroLightEffects')}{effectToggle('Floating dust effects', 'heroDustEffects')}{effectToggle('Studio LED effects', 'heroLedEffects')}
      {settings.heroVideoUrl && <div style={{ position: 'relative', marginTop: 18, overflow: 'hidden', borderRadius: 12, minHeight: 360, background: '#000' }}><video src={settings.heroVideoUrl} poster={settings.heroPosterUrl || undefined} muted loop autoPlay playsInline controls style={{ width: '100%', height: 420, objectFit: 'cover', display: 'block' }} /><div style={{ position: 'absolute', inset: 0, background: `rgba(0,0,0,${settings.heroOverlayOpacity / 100})`, pointerEvents: 'none' }} /><div style={{ position: 'absolute', inset: 0, display: 'grid', placeContent: 'center', textAlign: 'center', pointerEvents: 'none' }}><img src="/images/branding/Aureon_Header_Logo.png" alt="Aureon Music Group overlay preview" style={{ width: `${Math.min(settings.heroLogoScale, 130) * 3.8}px`, maxWidth: '72vw', height: 'auto' }} /><p style={{ letterSpacing: '.45em', fontWeight: 700, marginTop: 16 }}>CREATING TOMORROW’S CLASSICS</p></div></div>}
    </fieldset>
