...
function attributionRuntime(url,referrer=''){
 ...
 const context={module,exports:module.exports,require,URL,URLSearchParams,Date,console,crypto:{randomUUID:crypto.randomUUID},CustomEvent:class{},localStorage,sessionStorage,location,document:{referrer},window};
...
test('GA4 is single, consent-gated, environment configured and strips sensitive return parameters',()=>{const bridge=read('components/AnalyticsBridge.tsx'),firebase=read('lib/firebase-client.ts');assert.match(bridge,/NEXT_PUBLIC_GA_MEASUREMENT_ID/);assert.match(bridge,/send_page_view:false/);assert.match(bridge,/process\.env\.NODE_ENV!==\s*'production'/);assert.match(bridge,/ANALYTICS_CONSENT_KEY/);assert.doesNotMatch(bridge,/session_id/);assert.doesNotMatch(firebase,/G-[A-Z0-9]+/)});
