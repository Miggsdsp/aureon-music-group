...
export function MerchStore(){
 ...
 const products = records.map((item)=>({ ...item, image: item.imageUrl || item.image || '/images/branding/Aureon_Header_Logo.png', digital:false } as Product));
 const categories = ['All', ...Array.from(new Set(products.map((product)=>product.category).filter(Boolean)))] as string[];
 const [user,setUser]=useState<User|null>(null);const[member,setMember]=useState<Member|null>(null);
 const [category,setCategory]=useState('All');const[query,setQuery]=useState('');const[sort,setSort]=useState('featured');const[cart,setCart]=useState<CartItem[]>([]);const[cartOpen,setCartOpen]=useState(false);
 ...
 const filtered=useMemo(()=>{let list=products.filter(p=>(category==='All'||p.category===category)&&(`${p.name} ${p.artist||''}`.toLowerCase().includes(query.toLowerCase())));if(sort==='low')list=[...list].sort((a,b)=>a.price-b.price);if(sort==='high')list=[...list].sort((a,b)=>b.price-a.price);if(sort==='name')list=[...list].sort((a,b)=>a.name.localeCompare(b.name));return list;},[products,category,query,sort]);
 const itemCount=cart.reduce((sum,item)=>sum+item.quantity,0);const subtotal=cart.reduce((sum,item)=>sum+(discountedPrice(item.product.price)*item.quantity),0);
 function addToCart(product:Product){setCart(current=>{const existing=current.find(item=>item.product.id===product.id);if(existing)return current.map(item=>item.product.id===product.id?{...item,quantity:item.quantity+1}:item);return [...current,{product:{...product,digital:false},quantity:1,size:product.sizes?.[0],colour:product.colours?.[0]}];});setCartOpen(true)}
