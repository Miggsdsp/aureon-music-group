...
const SELECTORS: Array<[string,string,string]> = [
  ['.site-trust-bar','global_trust_bar','global'],
  ...
  useEffect(()=>{
    ...
    const observer=new IntersectionObserver(entries=>{
      for(const entry of entries){
        ...
        if(!placement||sessionStorage.getItem(key(placement)))continue;
    ...
    const click=(event:MouseEvent)=>{
      ...
      const placement=trust?.dataset.trustPlacement||(/membership|subscribe|choose/i.test(text)?'membership_cta':/buy|purchase|checkout/i.test(text)?'purchase_cta':/create account|join free/i.test(text)?'account_cta':'');
      if(!placement)return;
      trackAnalytics({eventType:conversion?'trust_conversion':'trust_click',entityType:'trust',entityId:placement,title:text,metadata:{trustPlacement:placement,trustCategory:trust?.dataset.trustCategory||'cta',conversionType:conversion?(/account|join/i.test(text)?'account_creation':/buy|purchase|checkout/i.test(text)?'purchase':'membership'):'click'}});
    };
