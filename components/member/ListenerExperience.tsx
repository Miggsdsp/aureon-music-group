...
import { Heart, History, Play, RotateCcw, Star } from 'lucide-react';
import { firebaseAuth, firestore } from '@/lib/firebase-client';
import { type PlayerSong, useMusicPlayer } from '@/components/music/MusicPlayerProvider';
import './listener-experience.css';

type Activity={favouriteSongIds:string[];favouriteArtists:string[];continueListening:any;recentlyPlayed:any[]};
const emptyActivity:Activity={favouriteSongIds:[],favouriteArtists:[],continueListening:null,recentlyPlayed:[]};
...
export default function ListenerExperience(){
 ...
 const refresh=useCallback(async()=>{if(!user)return;try{const data=await request() as Record<string,any>;setActivity({favouriteSongIds:Array.isArray(data?.favouriteSongIds)?data.favouriteSongIds:[],favouriteArtists:Array.isArray(data?.favouriteArtists)?data.favouriteArtists:[],continueListening:data?.continueListening||null,recentlyPlayed:Array.isArray(data?.recentlyPlayed)?data.recentlyPlayed:[]});setMessage('')}catch(error){setMessage(error instanceof Error?error.message:'Unable to load your library activity.')}finally{setLoading(false)}},[request,user]);
 ...
 return <section className="listener-experience" aria-label="Listener membership benefits">
  <div className="listener-benefit-head"><div><p>Listener membership</p><h2>Your personal library</h2></div></div>
  ...
  {loading?<div className="listener-loading">Loading your listening activity…</div>:<>
   ...
   <div className="listener-columns"><div className="listener-panel"><h3><History/> Recently played</h3>{activity.recentlyPlayed.length?activity.recentlyPlayed.slice(0,8).map(item=><button className="history-row" key={item.id} onClick={()=>songMap.get(item.songId)&&playSong(songMap.get(item.songId)!)}><span><strong>{item.title||'Untitled track'}</strong><small>{item.artist||'Aureon Music Group'}</small></span><Play/></button>):<p>No listening history yet. Play a track to begin.</p>}</div></div>
