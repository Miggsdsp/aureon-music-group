...
export function Header() {
  ...
  const links: NavigationLink[] = features.merchandiseEnabled
    ? [...baseLinks.slice(0, 6), [value.merchLabel, value.merchHref], ...baseLinks.slice(6)]
    : baseLinks;
  ...
  return (
    <header className="site-header premium-label-header">
      ...
      <div className="header-actions">
        ...
        <Link href={value.accountHref} className="header-icon" aria-label={value.accountLabel} title={value.accountLabel}>
          <UserRound size={20} />
        </Link>
        <Link href={value.membershipHref} className="join-button listen-button">
          {value.listenLabel} <Volume2 size={16} />
        </Link>
      </div>
    </header>
  );
