...
export function Footer() {
  ...
  const { items:legalDocuments } = usePublishedCollection<LegalRecord>('legalDocuments',[]);
  ...
  return (
    <footer className="aureon-footer">
      ...
      {legalLinks.length > 0 && <div className="aureon-footer-legal"><p className="aureon-footer-kicker">Legal Centre</p><nav aria-label="Legal documents">{legalLinks.map(item=><Link key={item.id} href={`/legal/${item.slug||item.id}`}>{item.title||'Legal document'}</Link>)}</nav></div>}
