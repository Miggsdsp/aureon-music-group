 ...
 return <AdminShell>
  ...
  <section className="admin-cms-grid">
   <form className="admin-cms-form" onSubmit={saveItem}>
    ...
    {(title==='Songs'||title==='Albums')&&<label>Artist<select required value={relations.artistId} onChange={e=>setRelations({...relations,artistId:e.target.value,albumId:''})}><option value="">Select artist</option>{artists.map(a=><option key={a.id} value={a.id}>{a.name||a.title}</option>)}</select></label>}
    {title==='Songs'&&<label>Album (optional)<select value={relations.albumId} onChange={e=>setRelations({...relations,albumId:e.target.value})}><option value="">Single / no album</option>{availableAlbums.map(a=><option key={a.id} value={a.id}>{a.title}</option>)}</select></label>}
   ...
   <div className="admin-cms-list"><table><thead><tr><th>Name</th>{(title==='Songs'||title==='Albums')&&<th>Artist</th>}<th>Status</th>{config.supportsPrice&&<th>Price</th>}<th>Actions</th></tr></thead><tbody>{items.length===0&&<tr><td colSpan={config.supportsPrice?5:4}>No records yet.</td></tr>}{items.map(item=><tr key={item.id}><td><strong>{item.name||item.title||'Untitled'}</strong><br/><small>{item.slug||item.id}</small>{title==='Songs'&&item.albumTitle&&<><br/><small>{String(item.albumTitle)}</small></>}</td>{(title==='Songs'||title==='Albums')&&<td>{String(item.artistName||item.details?.artistName||'Unassigned')}</td>}<td><span className="admin-status">{item.status||'active'}</span></td>{config.supportsPrice&&<td>€{Number(item.price||0).toFixed(2)}</td>}<td><div className="admin-row-actions"><button type="button" onClick={()=>startEdit(item)}>Edit</button>{config.supportsPublishing&&<button type="button" onClick={()=>togglePublish(item)}>{item.status==='published'?'Hide':'Publish'}</button>}<button type="button" onClick={()=>removeItem(item.id)}>Delete</button></div></td></tr>)}</tbody></table></div>
  </section>
 </AdminShell>
}
