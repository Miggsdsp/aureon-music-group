...
export function NewsManager(){
 ...
 return <AdminShell>
  ...
  <section className="admin-cms-grid">
   ...
   <div className="admin-table-wrap"><table><thead><tr><th>Article</th><th>Category</th><th>Featured</th><th>Status</th><th>Actions</th></tr></thead><tbody>{items.length?items.map(item=><tr key={item.id}><td>{item.title||'Untitled'}</td><td>{item.category||'—'}</td><td>{item.featured?'Yes':'No'}</td><td>{item.status||'draft'}</td><td><button type="button" onClick={()=>edit(item)}>Edit</button>{item.status!=='published'&&<button type="button" onClick={()=>{edit(item);setTimeout(()=>void save('published'),0)}}>Publish</button>}<button type="button" onClick={()=>{if(confirm('Delete permanently?'))void deleteDoc(doc(firestore,'newsArticles',item.id))}}>Delete</button></td></tr>):<tr><td colSpan={5}>No articles yet.</td></tr>}</tbody></table></div>
  </section>
 </AdminShell>
}
