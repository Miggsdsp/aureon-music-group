...
import { AUREON_STREAM_FORMAT, buildAacStreamingFile, buildWavPreviewFile } from '@/lib/audio-stream-encoder';
...
const fallbackArtists:RecordData[] = builtInArtists.map(artist=>({ id:artist.id,name:artist.name,slug:artist.slug,genre:artist.genre,status:'published',description:artist.desc }));
...
export function SongAdminSection(){
 ...
 async function uploadArtwork(file:File){const slug=form.slug.trim()||makeSlug(form.primary||'song');const path=`public/songs/covers/${slug}-${Date.now()}-${safeFileName(file.name)}`;try{const coverImageUrl=await uploadToStorage(path,file,'coverImageUrl');setUploadedDetails(current=>({...current,coverImageUrl}));setMessage('Artwork uploaded.');}catch(error){setMessage(error instanceof Error?error.message:'Artwork upload failed.');}}
 async function uploadMaster(file:File){
  if(masterProcessing){setMessage('A WAV master is already being processed. Please wait for it to finish.');return;}
  if(!relations.artistId){setMessage('Select an artist before uploading the WAV master.');return;}
  const isWav=/\.wav$/i.test(file.name)||['audio/wav','audio/x-wav','audio/wave'].includes(file.type);if(!isWav){setMessage('Upload the original WAV master only. Aureon creates the streaming and preview files automatically.');return;}
  const slug=form.slug.trim()||makeSlug(form.primary||'song');const artistFolder=String(selectedArtist?.slug||makeSlug(String(selectedArtist?.name||'unassigned')));const stamp=Date.now();const wavFile=file.type==='audio/wav'?file:new File([file],file.name,{type:'audio/wav'});
  setMasterProcessing(true);setMasterProgress(1);setMasterStage('Reading WAV master');setUploadedDetails(current=>{const next={...current};delete next.privateFilePath;delete next.streamFilePath;delete next.previewUrl;return next;});
  try{
   setMessage('Processing WAV master…');setMasterStage('Creating 256 kbps AAC subscriber stream');
   const streamFile=await buildAacStreamingFile(wavFile,slug,percent=>setMasterProgress(Math.max(1,Math.round(percent*0.42))));
   setMasterProgress(43);setMasterStage('Creating 40-second public preview');
   const previewFile=await buildWavPreviewFile(wavFile,slug,40);
   setMasterProgress(47);setMasterStage('Uploading protected WAV master');
   const privateFilePath=await uploadToStorage(`private/full-tracks/${artistFolder}/${slug}-${stamp}-master.wav`,wavFile,'privateFilePath',true,percent=>setMasterProgress(47+Math.round(percent*0.28)));
   setMasterStage('Uploading protected AAC subscriber stream');
   const streamFilePath=await uploadToStorage(`private/streams/${artistFolder}/${slug}-${stamp}-stream.aac`,streamFile,'streamFilePath',true,percent=>setMasterProgress(75+Math.round(percent*0.15)));
   setMasterStage('Uploading 40-second public preview');
   const previewUrl=await uploadToStorage(`public/previews/${artistFolder}/${slug}-${stamp}-preview.wav`,previewFile,'previewUrl',false,percent=>setMasterProgress(90+Math.round(percent*0.10)));
   setUploadedDetails(current=>({...current,privateFilePath,masterFormat:'WAV',streamFilePath,streamFormat:AUREON_STREAM_FORMAT,streamBitrateKbps:256,previewUrl,previewDuration:40}));setMasterProgress(100);setMasterStage('WAV master ready to save');setMessage('WAV master secured. AAC stream and 40-second preview created successfully. You can now save the song.');
